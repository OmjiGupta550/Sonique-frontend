...
export default function DiagnosePage() {
  ...
  const [results, setResults] = useState([
    ...
    },
    {
      ...
      status: "loading",
      message: "Checking current login session...",
    },
    {
      step: "Profiles Table Access",
      status: "loading",
      message: "Checking user database profile...",
    },
    {
      step: "Preferences Table Access",
      status: "loading",
      message: "Checking database preferences...",
    },
  ]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      // Load initial logs
      setLogs(window.youtubePlayerLogs || []);
  ...
  const checkYouTubeState = () => {
    ...
    const ytPresent = !!(window.YT && window.YT.Player);
    ...
    const isPlayerCreated = !!(
      container &&
      iframe &&
      (iframe.tagName === "IFRAME" || iframe.hasChildNodes())
    );
  ...
  const runDiagnostics = async () => {
    ...
    if (isPlaceholder) {
      ...
      newResults.push({
        ...
        status: "success",
        message:
          "Supabase client initialized successfully with custom credentials.",
    ...
    try {
      ...
      if (user) {
        ...
        newResults.push({
          ...
          details:
            "Please go to /login and sign in or sign up first, then return to this diagnostics page.",
        });
      }
    } catch (e) {
      newResults.push({
        ...
        if (profileError) {
          ...
          if (insertError) {
            newResults.push({
              ...
              status: "error",
              message:
                "Failed to insert profile row. RLS policies or trigger might be blocking.",
        ...
        if (prefsError) {
          ...
          if (insertPrefsError) {
            ...
            newResults.push({
              ...
              status: "success",
              message:
                "Successfully self-healed! Default preferences row created.",
            });
          }
        }
      } catch (e) {
  ...
  const getStatusIcon = (status) => {
    if (status === "loading") {
      return (
        <div className="w-4 h-4 rounded-full border border-t-transparent animate-spin border-zinc-400 shrink-0" />
      );
    }
    return status ? (
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 py-6 select-none pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400">
              Troubleshoot database connections and media streaming playback
            </p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          <h2 className="text-md font-bold text-zinc-300 flex items-center gap-2">
            <Database className="w-4 h-4 text-zinc-400" /> Database & Session
            Checks
          </h2>
          <div className="space-y-3">
            {results.map((res, index) => (
              <div
                key={index}
                className="bg-zinc-900/30 border border-white/5 p-4 rounded-xl flex items-start gap-4"
              >
          ...
          <div className="bg-zinc-900/30 border border-white/5 p-5 rounded-xl space-y-4">
            ...
            <div className="space-y-3 pt-2">
              ...
              <div className="text-[10px] text-zinc-500 leading-normal flex items-start gap-1">
                ...
                <span>
                  Note: To test correctly on desktop and mobile, click **Force
                  Init Player** first to verify browser authorization.
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <h2 className="text-md font-bold text-zinc-300 flex items-center gap-2">
          <FileText className="w-4 h-4 text-zinc-400" /> Live Streaming Engine
          Console Logs
        </h2>
        <div className="bg-black/60 border border-white/5 p-4 rounded-xl font-mono text-[10px] text-zinc-300 h-64 overflow-y-auto flex flex-col gap-1.5 leading-normal border-l-2 border-l-indigo-500/50 shadow-inner">
          {logs.length === 0 ? (
            <div className="text-zinc-600 italic h-full flex items-center justify-center">
              No logs recorded yet. Initialize the player or start playback to
              inspect traces...
            </div>
          ) : (
            logs.map((log, i) => (
              <div
                ...
                className={`py-0.5 border-b border-white/5 border-dashed last:border-b-0 ${
                  log.includes("Error") || log.includes("failed")
                    ...
                    : log.includes("onReady") || log.includes("PLAYING")
                      ? "text-emerald-400"
                      : log.includes("Setting src") ||
                          log.includes("Play requested")
                        ? "text-cyan-300"
                        : "text-zinc-300"
                }`}
              >

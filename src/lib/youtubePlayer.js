...
function logMsg(msg) {
  ...
  if (typeof window !== "undefined") {
    window.youtubePlayerLogs = window.youtubePlayerLogs || [];
    window.youtubePlayerLogs.push(formatted);
    window.dispatchEvent(
      new CustomEvent("youtube_player_log", { detail: formatted }),
    );
  }
}
...
class YouTubeAudioElement {
  ...
  _muted = false;
  intervalId = null;
  elementId = "hidden-youtube-player-iframe";
  containerId = "hidden-youtube-player-container";
  ...
  initPlayer() {
    ...
    const pollYT = setInterval(() => {
      if (window.YT && window.YT.Player) {
        logMsg(
          "Polling resolved: window.YT.Player is ready. Re-initializing...",
        );
    ...
    if (
      ...
      if (firstScriptTag && firstScriptTag.parentNode) {
        ...
        logMsg(
          "No existing script tags found. Appended script to document head.",
        );
      }
    } else {
      ...
      this.player = new window.YT.Player(this.elementId, {
        ...
        events: {
          ...
          onError: (e) => {
            ...
            logMsg(
              "Player error detected. Triggering ended to automatically skip track.",
            );
            this.trigger("ended");
          },
        },
  ...
  set src(val) {
    ...
    if (this.videoId !== vidId) {
      logMsg(
        `Changing video ID from "${this.videoId}" to "${vidId}". Resetting currentTime and duration.`,
      );
      this.videoId = vidId;
      this._currentTime = 0;
      this._duration = 0;
  ...
  play() {
    logMsg(
      `Play requested. isReady=${this.isReady}, videoId="${this.videoId}", isLoaded=${this.isLoaded}`,
    );
    this.pendingPlay = true;
    if (this.isReady && this.player && this.videoId) {
      if (!this.isLoaded) {
        logMsg(
          `First-load for video ID: ${this.videoId}. Invoking loadVideoById with offset: ${this._currentTime}`,
        );
...
export function createYouTubeAudioElement() {
  return new YouTubeAudioElement();
}

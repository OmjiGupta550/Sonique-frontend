...
export function cleanTitleAndArtist(title, artist) {
    ...
    try {
      ...
      return items
        ...
        .map((item) => {
          ...
          const { cleanTitle, cleanArtist } = cleanTitleAndArtist(
            item.title,
            item.uploaderName,
          );
          return {
...
export async function getDirectAudioStream(videoId) {
  let attempts = 0;
  while (attempts < PIPED_INSTANCES.length) {
    const instance = getPipedInstance();
    try {
      ...
      if (audioStreams.length > 0) {
        ...
        const selected =
          m4aStreams.length > 0 ? m4aStreams[0] : audioStreams[0];
        return selected.url || null;
      }
    } catch (err) {
      console.warn(
        `Piped stream extraction from ${instance} failed. Retrying...`,
        err,
      );
      rotateInstance();
      attempts++;
